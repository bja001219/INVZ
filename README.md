...
**환경변수 목록** → [환경변수](#환경변수) ·
**설계 설명** → [설계](#설계) ·
**테스트·검증 방법** → [리뷰어 검증 방법](#리뷰어-검증-방법) · [AI 산출물 검증 방법](#ai-산출물-검증-방법).
...
위 자동화된 테스트는 실제 OpenAI나 Kie API에 도달하지 않으므로 API 키 없이 그대로 통과합니다.
`pytest-socket`이 루프백을 제외한 모든 호스트를 차단하고, 프로바이더 계약은 `respx` HTTP 스텁으로
고정합니다. 그러니 자동화된 스위트가 통과한다는 사실이 Live 경로가 실제로 동작한다는 뜻은 아닙니다 —
Live는 사람이 눌러서 확인해야 하며, 그 절차는 다음 절의 6번 규칙에 있습니다.

## AI 산출물 검증 방법

이 저장소는 코드 대부분을 AI 에이전트가 작성했습니다. AI가 만든 코드는 그럴싸해 보이는 만큼 조용히
틀리기 쉬워서, 사람이 그 사이를 좁히기 위한 규칙 몇 개를 [`AGENTS.md`](AGENTS.md)에 박아 두고 커밋마다
지켰습니다. 규칙은 취향이 아니라 각각이 이 저장소에서 실제 결함을 만들어 낸 뒤에야 추가된 것입니다.

### 1. 실패 테스트를 먼저 쓴다

구현보다 실패하는 테스트를 먼저 씁니다. 실패 메시지를 눈으로 읽고 그것이 내가 만들려던 실패가
맞는지 확인한 뒤에야 최소 구현을 씁니다. 그다음 영향받는 스위트를 다시 돌립니다. 사후에 붙은
테스트는 "코드가 돈다"만 증명하지 "코드가 옳다"를 증명하지 않습니다. 앵커 게이트,
`SUBMISSION_UNCERTAIN` 정리, 웹훅 idempotency 가드 같은 상태 기계 로직은 전부 이 순서로 들어왔습니다.

### 2. reasoning이 아니라 output으로 검증한다

"이 코드는 이렇게 동작할 것이다"라는 설명은 증거로 인정하지 않습니다. 실행한 명령과 그 결과를
붙여야만 검증으로 칩니다. 이 규칙은 실제 사건 뒤에 추가됐습니다. 로그 마스킹 변경이 "잘 동작한다"고
보고된 적이 있는데, 근거로 삼은 로그 한 줄이 알고 보니 로거 자체의 *에러 덤프*였습니다. 문자열은
맞게 보였고 메커니즘은 부서져 있었습니다. 수정은 `58f026b fix: redact log arguments in place instead
of collapsing them`에 남아 있고, 이후로 마스킹 관련 변경에는 실제 액세스 로그와 웹훅 요청 캡처가
같이 붙습니다.

### 3. 설계 문서를 코드보다 먼저 쓴다

각 기능은 [`docs/superpowers/specs/`](docs/superpowers/specs/)에 설계 스펙이 들어간 뒤
[`docs/superpowers/plans/`](docs/superpowers/plans/)에 구현 계획이 붙고, 코드는 그다음입니다.
"프롬프트만 잘 지어" AI에게서 코드를 얻는 방식은 결정이 어디에 있는지 아무도 되짚을 수 없게 만듭니다.
스펙이 먼저 있으면 AI는 그것을 참조 자료로 물고, 다음 세션과 리뷰어는 왜 이 형태인지 되돌아 볼
지점을 얻습니다. 두 문서는 결정이 바뀔 때마다 함께 갱신됩니다.

### 4. 대체된 결정은 같은 커밋에서 지운다

코드 변경이 README나 설계 문서를 흔들면 문서를 **같은 커밋에서** 갱신하고 무엇으로 대체됐는지
남깁니다. 이 저장소는 한동안 "런타임 모드 스위치는 없다"고 적힌 README와 그 40줄 아래에 있는
`PUT /api/config` 라우트를 동시에 담고 있었습니다. 스테일한 문서가 남으면 다음 AI 세션이 그 문서를
사실로 받아들여 잘못된 방향으로 뜁니다. 커밋 `a9c0504 docs: add the required agent instructions and
correct the stale README claims`가 그 뒷정리입니다.

### 5. 커밋 전 두 개의 grep 스캔

**둘 다 아무 것도 출력하지 않아야** 커밋합니다. 패턴 문자열 자체를 인용하는 README와 계획 문서는
제외합니다.
...
첫 스캔은 AI가 코드·로그·주석·픽스처 어디에도 실제 API 키 형태의 문자열을 남기지 않았는지 확인합니다.
두 번째는 설계에서 빠진 이름들이 코드에 좀비처럼 남아 있지 않은지 확인합니다. 리팩터링을 지시한 뒤에도
옛 이름이 어딘가에 남는 일이 종종 있고, 이 스캔은 그 잔재를 잡습니다. 한때 `/api/webhooks`도 이
목록에 있었지만 지금은 살아 있는 라우트라서 제거했습니다. 그대로 두면 스캔이 항상 매치를 뱉어, 아무도
읽지 않는 검증이 됩니다.

`git grep`은 **추적되는 파일만** 훑습니다. "커밋된 비밀값 없음"은 보장하지만 "작업 트리에 비밀값
없음"은 보장하지 않습니다. `.env`와 `backend/.env`는 `.gitignore`에 걸려 이 스캔의 사각지대에 있으니
로컬 파일에 실제 키를 두었다면 다른 방법으로 지켜야 합니다.

### 6. Live 경로는 사람이 직접 눌러 본다

자동화된 테스트는 실제 프로바이더에 도달할 수 없으므로 (앞 리뷰어 절 참조) Live 모드는 자동으로
증명되지 않습니다. 대신 실제 키를 넣고 단일 Cut 버튼으로 이미지 하나·영상 하나를 실제 프로바이더에
쏘아 본 뒤, 요청 페이로드·응답·저장된 URL·재생 결과를 눈으로 확인합니다. 이 과정에서 `kling-2.6`의
실측 지연이 3분 30초~4분이라는 사실을 발견해 `GENERATION_ATTEMPT_TIMEOUT_SEC` 기본값을 120초에서
300초로 올렸습니다. 그전 기본값은 정상 지연 도중에 만료돼 이미 청구된 영상을 두 번 청구시켰습니다.
AI가 처음 잡은 120초는 스위트를 다 통과했지만, 사람 눈으로 눌러 보고 나서야 잘못됐다는 것이
드러났습니다.

### 7. 루프가 실제로 결함을 잡는다는 증거

위 규칙들이 무엇을 잡았는지는 git log에 그대로 남습니다. 아래 커밋들은 각 자리에 결함이 있었다는
증거이자, 규칙이 이론이 아니라는 증거입니다. `git show <hash>`로 진단과 수정을 함께 확인할 수 있습니다.

- `f49276c fix: close the anchor gate and the two claim-path defects behind it` — 앵커 게이트가
  단일 Cut 경로에서 우회되던 버그. 실패 테스트가 첫 우회를 잡자 그 옆에 있던 두 개의 claim-path
  결함이 함께 드러났습니다.
- `295997a fix: close the webhook/poll race and stop leaking a live provider client` — 웹훅과 폴링이
  같은 job에 결과를 동시에 실어 두 번째 산출물을 만들 수 있던 경합. 상태 전이 가드가 두 실패 분기에
  빠져 있어 발생했고, 지금은 다섯 분기 전부가 같은 재조회 가드를 지납니다.
- `366d4d3 fix: compare webhook credentials as bytes so a non-ASCII token cannot 500` — 비ASCII
  토큰이 `compare_digest`에서 `TypeError`로 500을 유발했습니다. 헤더 바이트 시퀀스를 그대로 실은
  실패 테스트가 잡아냈습니다.
- `1f57b5d fix: retry a scene draft the model shaped wrong` — 모델이 스키마가 거부하는 형태로
  응답한 경우를 영구 실패로 두지 않고 재시도 가능한 `SchemaProviderError`로 다뤄, 초안 한 번 삐끗한
  것이 502로 나가지 않게 했습니다.
